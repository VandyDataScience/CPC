...
#Predictive Model for Awareness of CPC:
def runAwarenessModel (awareness) : 
    ...
    X = local[["How often do you visit Centennial Park?", "What is your age?", "What is your gender?", "How far do you travel to get to the Park?"]]
    X = pd.get_dummies(data=X, drop_first=True)
    local = local.assign(column = [1 if str(a).__contains__(awareness) else 0 for a in local["Were you aware that there is a nonprofit support group, Centennial Park Conservancy, that helps to underwrite Park improvements and arts & educational programs in the Parthenon and Park through donations and memberships?"]])
    y = local["column"]
...
#Model to predict why people visit the park:
def runModel (activity) : 
    local = df.loc[(df["How far do you travel to get to the Park?"]=="Less than a mile away") | (df["How far do you travel to get to the Park?"]=="6 - 9 miles away") | (df["How far do you travel to get to the Park?"]=="1 - 5 miles away")]
    X = local[["How often do you visit Centennial Park?", "What is your age?", "What is your gender?"]]
    X = pd.get_dummies(data=X, drop_first=True)
    local = local.assign(column = [1 if str(a).__contains__(activity) else 0 for a in local["Why do you visit the Park? (check all that apply)"]])
    y = local["column"]
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
    model = LogisticRegression()
    model.fit(X_train, y_train)
...
#Run predictive model for each reason why locals visit the park:
runModel("Spend time in an outdoor green space")
runModel("Exercise (walk, bike, run)")
runModel("Visit the Parthenon")
runModel("Visit Centennial Arts Center")
runModel("Visit Centennial Performing Arts Studios")
runModel("Attend events")
runModel("Picnic")
runModel("Walk dog")
runModel("Visit the dog park")
runModel("Feed the ducks")
runModel("Visit the playground")
runModel("Use the volleyball courts")
runModel("Play sports")
